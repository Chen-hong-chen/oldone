...
extern gps  gps_buff;
//串口发送缓存区 	
 uint8_t USART2_TX_BUF[USART_MAX_SEND_LEN]; 	//发送缓冲,最大USART_MAX_SEND_LEN字节
...
{			 
	uint16_t len=0,i;
	uint8_t rval=0;
	while(UartRxLen==0 && len<100)//等待接收到应答   
	{
		len++;
		HAL_Delay(5);
	}		 
...
void Gps_Msg_Show(void)
{
	...
	tp=gpsx.longitude;	   
	//sprintf((char *) gps_buff.jingdu,":%.5f",tp/=100000,gpsx.ewhemi);//得到经度字符串
        
         gps_buff.jingdu=(int)((tp/=100000) * 1e6) / 1e6;
...
//	OLED_ShowString(32,2,dtbuf);
	...
	//sprintf((char *)dtbuf,":%.5f %1c",tp/=100000,gpsx.nshemi);	//得到纬度字符串
       gps_buff.weidu=(int)((tp/=100000) * 1e6) / 1e6;
...
{
//  memset(&gps_buff,ZE,sizeof(gps_buff));//清楚其gps中错误数据，防止后续取地址时遇到干扰
//   uint8_t key=0XFF;
// HAL_UART_Receive_IT(&huart2, (uint8_t *)&UartRxData, 1);//接收中断使能
//  osDelay(500);
